                     ...
                     BYTE_T *spareCap)
{
	...
	//DBD-honma 07Jul2004
	BYTE_T expectCnts;
	BYTE_T lastDemandPos,demandFlag;
            ...
            for (floor=start; floor<=stop; floor++)
               {
                  ...
                  demandFlag = FALSE;
                   ...
                   if (((asslib_DbdExpectCalls[car][floor] & dest_mask) != 0) &&
                      ((dbdFlag == FALSE) || 
                      //DBD-honma 07Jul2004
                      ((dbdFlag) && (asslib_ValidExpectCalls(car,dest_dir,floor,&expectCnts)))))
                   {
                       /* DBD-honma 07Jul2004
                       if (*spareCap > asslib_DbdExpectCallCnts[car][dest_dir][floor]) 
                           *spareCap -= asslib_DbdExpectCallCnts[car][dest_dir][floor];
                       */
                       if (dbdFlag == FALSE) expectCnts = asslib_DbdExpectCallCnts[car][dest_dir][floor];
                       if (*spareCap > expectCnts) 
                          *spareCap -= expectCnts;
                       else                    
                          *spareCap = 0;
                   ...
                   //DBD-honma 09Jul2004          
                   if ((ocss_info[car].ezStandardFlag == TRUE) && (demandFlag == TRUE))
                   {
                       ica_rat_pwt += asslib_CheckEzRunTime(car,lastDemandPos,floor);
                       lastDemandPos = floor;
                   }
                  ...
                  if ((ocss_info[car].assigned_calls[floor] & (S_UHC+E_UHC)) != 0)
                    {
                     ...
                     if ((ocss_info[car].assigned_calls[floor] & S_FUHC) != 0)
                       {
                          //DBD-honma 04Apr04
                          //DBD-honma MixConfig   15Jun2004
                          if ((*e2p_DbdAssignEnable == 0) ||
                             ((*e2p_DbdAssignEnable != 0) && (DBD_GetFloorDecCount(floor) == 0)))
                          {
...
//dojima
                     ...
                     if (floor != stop)
                       {
                         if ((ocss_info[car].assigned_calls[floor]  & E_UHC) != 0)
                           {
                            // DBD-JAS may need to add conditional logic here for OTI/SAPA  ica_rat_pwt += RRT_WCS_STOP;
                              ica_rat_pwt += rrt_hdcp_hall_stop;
                              ica_rat_pwt += RRT_SHORT_RUN;
                           }
                        //DBD   else
                  ...
                  //DBD
                           ...
                           (((asslib_DbdExpectCalls[car][floor] & dest_mask) != 0) &&    
                            ((dbdFlag == FALSE) || 
                            //DBD-honma 07Jul2004
                            ((dbdFlag == TRUE) && (asslib_ValidExpectCalls(car,dest_dir,floor,&expectCnts))))) ||
                      ...
                      if (floor != stop)
                        {
                          if ((ocss_info[car].assigned_calls[floor] & E_DHC) != 0)
                            {
                          //DBD-JAS may need to bring back for OTI-OCSS/SAPA ica_rat_pwt += RRT_WCS_STOP;
                               ica_rat_pwt += rrt_hdcp_hall_stop;
                               ica_rat_pwt += RRT_SHORT_RUN;
                            }
                           //DBD      else 
