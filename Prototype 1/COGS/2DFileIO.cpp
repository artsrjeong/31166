						   ...
						   PASSENGER pax[MAXPAX],MOVE_PARAMETER move[MAXMOVE],int *paxCount,int *moveCount)
{
	...
	if (fin.good() !=0)
	{
		...
		*moveCount=movCount;
		...
		for (floorNo=0;floorNo<building->nUserFloors;floorNo++) // shs temp
		{
			fin >> intTemp; // floor
			for (carNo=0; carNo< (unsigned)building->nCars;carNo++)
			{
				fin >> setiosflags(ios::hex)>> intTemp;
				param->AllowMask[carNo][floorNo + building->pit]=intTemp;
			}
			fin.getline(charTemp,200);
		}
		if (building->pit)
			for (carNo=0; carNo< (unsigned)building->nCars;carNo++)
				param->AllowMask[carNo][0]=0;  // AH: needs to consider bits 16 and 17 eventually
		if (building->overhead)
			for (carNo=0; carNo< (unsigned)building->nCars;carNo++)
				param->AllowMask[carNo][building->nTotalFloors - 1]=0;  // AH: needs same as above
