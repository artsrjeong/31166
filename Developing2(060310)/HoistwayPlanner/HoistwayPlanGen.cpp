	...
	if ( feasible_sequence.sequence_start != -1 && feasible_sequence[firstStop].next == -1)
	{
		...
		while (callIndex != -1)
		{
			...
			// if ((Hcalls[callIndex].origin == firstFloor) && feasible_sequence[firstStop].empty )
			{
				...
				{
					start = firstFloor;
					int ind = Hcalls[callIndex].next;
					while (ind != -1)
					{
						// if found a call with the same direction and same first floor
						if (Hcalls[ind].origin == firstFloor && Hcalls[ind].direction() == curr_dir)
							break;
						ind = Hcalls[ind].next;
					}
					if (ind == -1) {
