...
struct Elevator* theElevator;

typedef struct Pkt {
    unsigned char carId;		/*## attribute carId */
    int curInx;		/*## attribute curInx */
    unsigned char decel;		/*## attribute decel */
    unsigned char door;		/*## attribute door */
    unsigned char motion;		/*## attribute motion */
    int nextInx;		/*## attribute nextInx */
} Pkt;
...
void updateFloorCall()
{
	...
	for(int floor=bdInfo.nLandings-1;floor>=0;floor--)
	{
...
void SpyCar(unsigned char car,int cmd_target,int cmd_door,int cmd_preDir)
{
	...
	if(car==0 || car==1)
	{
		Pkt pkt;
		pkt.carId=car;
		pkt.decel=ocss_info[car].decelDecided ? 1 : 0;
		pkt.door=ocss_info[car].frontDoorState;
		pkt.motion=ocss_info[car].motionState;
		pkt.curInx=ocss_info[car].curInx;
		if(ocss_info[car].curInx!=-1)
		{
			pkt.nextInx=ocss_info[car].stop[ocss_info[car].curInx].next;
		}
		else
		{
			pkt.nextInx=-1;
		...
		BYTE* lpMapAddress;
		lpMapAddress = (BYTE*)MapViewOfFile(hMapFile, // Handle to mapping object. 
		...
		int k=sizeof(Pkt);
		memcpy(lpMapAddress+2+sizeof(Pkt)*(int)pWrite,&pkt,sizeof(Pkt));
		*lpMapAddress=*lpMapAddress+1;
		pWrite=*lpMapAddress;
	}
}
...
#endif
#ifdef COGS_UNIT_TEST
	hMapFile = CreateFileMapping(NULL,    // Current file handle. 
...
__declspec(dllexport) void IHCInitialize(BuildingInfo *mBdInfo,HoistwayInfo mHwyInfo[],CarInfo mCarInfo[],Parameter *mParam,char* version)
{
	...
	theElevator = Elevator_Create(RiCMainTask());
	Elevator_startBehavior(theElevator);
	RiCOXFStart(TRUE);
*/
	//ABCD(mBdInfo);
}
