...
// Make break point table depending on parameter
...
// calculate jerk, accel, speed for that time.
...
// Get Break Point Table Index from time
...
//used by COGS_SPY to get ESP Absolute position
int CMotion::GetESPPos(double time,int *into_decel,int should_be_in_flight)
{
	...
	else
	{
		if (index == START_DELAY_PERIOD )
		{
			return (int)FloorToPos(origin);//No change in the motion state
		}
		if (index == STOP_DELAY_PERIOD)
		{
			return (int)FloorToPos(target);
		...
		{
			return (int)FloorToPos(origin);//add err log
		}
	}
	if(deltaT < 0.0)
	{
...
// Generate profile with numerical method
...
// Convert from Absolute position to NCF
...
// Time it takes from start to decel point
