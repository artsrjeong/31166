...
#include "..\GDInterfaceCOGS\GDInterfaceCOGS.h"
#include "NPI.h"
#undef EXTERN
...
// external functions
ReturnAssignmentToPassenger_T ReturnAssignmentToPassengerExt;
//AcceptAssignment_T AcceptAssignmentExt;
// ReturnHoistwayPlanScore_T ReturnHoistwayPlanScoreExt;  // CSW060104: ReturnHoistwayPlanScore is an internal function
// HPPlanParkingDLL_T HPPlanParkingDLL;
...
void GDInitialize(BuildingInfo *mBdInfo,
				  ...
				  char *versionName) {
	int h, c, k, f;
	HINSTANCE hinstCOGS, hinstIHC;		// CSW051213: Link to IHC instead of HP
...
// attach external functions
	...
	ReturnAssignmentToPassengerExt = (ReturnAssignmentToPassenger_T)
		GetProcAddress(hinstCOGS, "ReturnAssignmentToPassenger");
	if (ReturnAssignmentToPassengerExt == NULL) {
#ifdef WITH_DEBUG
		...
		hinstIHC = LoadLibrary(IHC_FILE);
	if (hinstIHC == NULL) {				// CSW051213: change to IHC DLL
#ifdef WITH_DEBUG
		fprintf(f_ErrorLog, "GDInitialize 3: cannot load IHC DLL as library\n");
#endif
		return;
	}

	AcceptAssignmentDLL = (AcceptAssignmentDLL_T) GetProcAddress(hinstIHC, "AcceptAssignmentDLL");  // CSW051213: Change to IHC DLL
	if (AcceptAssignmentDLL == NULL) {
#ifdef WITH_DEBUG
		fprintf(f_ErrorLog, "GDInitialize 4: cannot get proc address for AcceptAssignment\n");
#endif
		return;
	}
	// CSW051213: This function is part of GD DLL
	/*
	ReturnHoistwayPlanScoreExt = (ReturnHoistwayPlanScore_T)
		GetProcAddress(hinstHP, "ReturnHoistwayPlanScoreExt");
	if (ReturnHoistwayPlanScoreExt == NULL) {
#ifdef WITH_DEBUG
		fprintf(f_ErrorLog, "GDInitialize 5: cannot get proc address for ReturnHoistwayPlanScoreExt\n");
#endif
		return;
	}
	*/
	HPPlanParkingDLL = (HPPlanParkingDLL_T)
		GetProcAddress(hinstIHC, "HPPlanParkingDLL");		//CSW051213: Change to IHC DLL
	if (HPPlanParkingDLL == NULL) {
#ifdef WITH_DEBUG
...
extern "C" __declspec(dllexport)
void GDAlgorithmInitialize() {
	int h, c, f, n_serviceable, lower_sector_floors, top_of_lower_sector;
		...
		case 6:  // SimpleUpPeakDispatcher
			if (param.bParkingEnabled)
				for (c = 1; c < bdInfo.nCars; c++) {
					...
					//CSW060104 : Send parking specification to both DLL and local planner
					GiveParking((UCHAR) c, (FLOOR_T) 0, (FLOOR_T) 0, parking_location,
						(TIME_T) 0, (SHORT) 0);
...
// Use AlgorithmParam.int_param_1 to indicate the highest floor index of the lower sector
            n_serviceable = 0;  // count of serviceable floors
			for (f = 0; f < bdInfo.nLandings; f++)
                if (carInfo[0].callMask[f])
					n_serviceable++;

			top_of_lower_sector = (int) n_serviceable/2;
			lower_sector_floors = 0;
			for (f = 0; f < bdInfo.nLandings; f++)
                if (carInfo[0].callMask[f]) {
					if (lower_sector_floors == top_of_lower_sector) {
						AlgorithmParam.int_param_1 = f;
						break;
					}
					lower_sector_floors++;
				}
			...
			if (param.bParkingEnabled)
				for (h = 0; h < bdInfo.nHoistways; h++)
					if (h < bdInfo.nHoistways/2) {
						//CSW060104 : Send parking specification to both DLL and local planner
						GiveParking((UCHAR) 2*h, (FLOOR_T) 0, (FLOOR_T) 0,
							(FLOOR_T) 0, (TIME_T) 0, (SHORT) 0);
						HPPlanParking((UCHAR) 2*h, (FLOOR_T) 0, (FLOOR_T) 0,
							(FLOOR_T) 0, (TIME_T) 0, (SHORT) 0);
						GiveParking((UCHAR) 2*h + 1, (FLOOR_T) 0, (FLOOR_T) 0,
		...
		case 10:  // StaticU1U2D1D2A
			if (param.bParkingEnabled)
				//CSW060104 : Send parking specification to both DLL and local planner
				GiveParking((UCHAR) 0, (FLOOR_T) 0, (FLOOR_T) 0, (FLOOR_T) 0,
					(TIME_T) 0, (SHORT) 0);
				GiveParking((UCHAR) 1, (FLOOR_T) 0, (FLOOR_T) 0, (FLOOR_T) 1,
					(TIME_T) 0, (SHORT) 0);
				GiveParking((UCHAR) 2, (FLOOR_T) 0, (FLOOR_T) 0,
					(FLOOR_T) 1+(bdInfo.nLandings-1)/4, (TIME_T) 0, (SHORT) 0);
				GiveParking((UCHAR) 3, (FLOOR_T) 0, (FLOOR_T) 0,
					(FLOOR_T) 3*(bdInfo.nLandings-1)/4, (TIME_T) 0, (SHORT) 0);
...
// Store call in local data structure
	if (assignment >= 0) {
		...
		ReturnAssignmentToPassengerExt(sequenceNumber, assignment);
		// CSW051213: This function should call the external HP
		GiveAssignment(assignment, new_call, state, ctCurrentTime);
	}
}
							...
							TIME_T ctCurrentTime)
{
	...
	if (new_call->origin < new_call->destination)
	{
		if (new_call->destination <= AlgorithmParam.int_param_1)
			c = 0;
		else
			c = 1;
	}
	...
	{
		if (new_call->origin <= AlgorithmParam.int_param_1)
			c = 0;
		else
			c = 1;
	}
