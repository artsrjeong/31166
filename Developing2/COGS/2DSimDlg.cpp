...
//						Car[c].changed_target = 1;
			...
			if (scan_for_target) {                         // find new target from list of calls
				...
				if (run_to_new_target) {                     // run car to call location
					...
					{
						dist = fabs(FloorToPos(Car[c].target) - FloorToPos(Car[c].floor));
						// shin060202 Do not update the car state when the travel dist of the car is not exceeding 0.0
						if(dist <= 0.0)
							fprintf(f_ErrorLog, "UpdateTarget: dist is (%lf) (%d: %d -> %d)\n", dist, c, Car[c].floor, Car[c].target);
						else
						{
							GenerateFullProfile(c, dist);
							Car[c].t_decel = Car[c].t_run_start + GetCommittableTime(c, f, new_target);
...
void CMy2DSimDlg::HandleNewPassenger(int p)
{
	...
	if (m_bHMAttached==TRUE && m_bDispatcherAttached==FALSE)
	{
		...
		//shin050715
		AcceptAssignmentDLL(c,call,carState,currentTime); 
...
void CMy2DSimDlg::HandleCarOperation(int c)
{
	...
	if (m_bHMAttached) // shs0708 add put car state and issue monitoring
	{
		...
		PutCarStateDLL(c,&carState);
		other = GetOtherCarIndex(c);
		if(other != MAXCAR)
		{
			MakeUpOtherCarState(other, &carState);
			PutCarStateDLL(other, &carState);
		}
...
// Normal case: there is a reason to open the doors
			if ((Car[c].q_CCall[Car[c].floor] >= 0||	//There are passengers on the car c to current floor
				...
				&& !((UpdatedCarCommand[c] == TRUE) && (CarCommandFromHPE[c].frontDoorCommand == DOOR_NOT_OPEN)))//shin060127
			{
				SetPreDirection(c);
				Car[c].oper = OPENING;
				Car[c].n_board= CalculateExpectedBoarder(c);
...
//shin060207 Calculate the number of passengers in each car considering the status of the car.
void CMy2DSimDlg::CalculatePassengerInCars()
{
	...
	int dummy;
	int nPassenger=0;
	int nGettingOnOff[MAX_FLOOR];//shin060207
	int p;
	...
	for(c=0; c<n_car; c++)
	{
		...
		else
		{
			...
			if(Car[c].dir == UP)
			{
				...
				for(f=ncf; f<n_floor; f++)
				{
					if((Car[c].n_UpCall[f] > 0) /*&& (bGetOn)*/)
					{
						for(p = Car[c].q_UpCall[f]; p>0; p=Pax[p].q_next)
						{
							nGettingOnOff[f] +=1;
							nGettingOnOff[Pax[p].f_dest] -=1;
						}
					}
				}
				for(f=ncf; f<n_floor; f++)
				{
					nPassenger = nPassenger + nGettingOnOff[f] - Car[c].n_CCall[f];
				...
				for(f=n_floor-1; f>=0; f--)
				{
					if((Car[c].n_DnCall[f] > 0) /*&& (bGetOn)*/)
					{
						for(p = Car[c].q_DnCall[f]; p>0; p=Pax[p].q_next)
						{
							nGettingOnOff[f] +=1;
							nGettingOnOff[Pax[p].f_dest] -=1;
						}
...
void CMy2DSimDlg::CheckAssignments(int c, int flr, int dir, int p)
{
	...
	// H: 해당 층에 같은 호이스트웨이의 다른 카가 먼저 서비스를 하는가?
	if((Car[other].n_UpCall[flr] || Car[other].n_DnCall[flr]) ||
		...
		(((Car[c].oper == 1) || (Car[c].oper == 2) || (Car[c].oper == 3)) && (dir == -Car[c].dir) && Car[c].floor == flr))
	{
		...
		if(m_violationHCD & VIOLATION_H)
			DisplayWarningList(str);
		// HC: 해당 층에 해당 호이스트웨이의 해당카가 다른 방향으로 먼저 서비스를 하는가?
		...
		// HD: 해당 층에 같은 호이스트웨이의 다른 카가 먼저 같은 방향으로 서비스를 하는가?
	...
	// C: 해당 층에 다른 호이스트웨이의 같은 카가 먼저 서비스를 하거나 같은 호이스트웨이의 같은 카가 반대 방향으로 먼저 서비스를 하는가?
	for(i=0, bCond=FALSE; i<n_car; i++)
	{
		if( i == c )
			continue;
		...
		(((Car[c].oper == 1) || (Car[c].oper == 2) || (Car[c].oper == 3)) && (flr == Car[c].floor) && (dir == -Car[c].dir)))
	{
		...
		if(m_violationHCD & VIOLATION_C)
			DisplayWarningList(str);
		// CD: 해당 층에 다른 호이스트웨이의 같은 카가 같은 방향으로 먼저 서비스를 하는가?
		for(i=0; i<n_car; i++)
		{
			if( i == c )
				continue;
	...
	// D: 해당 층에 다른 호이스트웨이의 카 또는 같은 호이스트웨이의 다른 카가 같은 방향으로 먼저 서비스를 하는가?
	for(i=0; i<n_car; i++)
	{
		if( i == c )
			continue;
...
void CMy2DSimDlg::DetachIHC()
{
	if (hinstIHC != NULL)
	{
		AfxFreeLibrary(hinstIHC);
		hinstIHC=NULL;
	}
	m_bHMAttached = FALSE;
}
...
void CMy2DSimDlg::LinkDLLs()
{
	...
	if (m_bHM==TRUE)  // attach HM
	{
		...
		{
			if(LinkIHC(IHC_FILE) != TRUE)
			{
				//fail to load dll
				CString cstr;
				cstr.Format("Fail to load IHC.DLL");
				MessageBox(cstr);
				DetachIHC();
			}
			else
...
void CMy2DSimDlg::InitializeDLLs()
{
	...
	if (m_bHMAttached==TRUE)
	{
		...
		//shin060103
		strcpy(versionNameIHC,dispatcherName);
		IHCInitialize(&bdInfo, hwyInfo, carInfo, &parameter, versionNameIHC);
		// PutCarStateDLL after initializing the status
		CarState carState;
		for (car=0;car<bdInfo.nCars;car++)
		{
			MakeUpCarState(car,&carState);
			PutCarStateDLL(car,&carState);
		}
	}
...
// shin060103
// Link IHC
int CMy2DSimDlg::LinkIHC(CString IHCDLLFileName)
{
	BOOL bIHCAttached = FALSE;
	CString cstr;

	if (hinstIHC != NULL)
		AfxFreeLibrary(hinstIHC);
	hinstIHC=AfxLoadLibrary(IHCDLLFileName);

	if (hinstIHC != NULL)
	{
		IHCInitialize=(HPInitialize_T) GetProcAddress(hinstIHC,"IHCInitialize");
		PutCarStateDLL=(PutCarStateDLL_T) GetProcAddress(hinstIHC,"PutCarStateDLL");
		MonitorCarState=(MonitorCarState_T) GetProcAddress(hinstIHC,"MonitorCarState");
		// for manual dispatching to HP (not pass though dispatcher)
		AcceptAssignmentDLL=(AcceptAssignmentDLL_T) GetProcAddress(hinstIHC,"AcceptAssignmentDLL");

		if (IHCInitialize != NULL
			&& PutCarStateDLL != NULL
			&& MonitorCarState != NULL
			&& AcceptAssignmentDLL != NULL)
		{
			bIHCAttached=TRUE;
		}
	}

	if (bIHCAttached == FALSE)
	{
		if (hinstIHC != NULL)
			FreeLibrary(hinstIHC);
		cstr.Format("Fail to Load IHC.dll");
		MessageBox(cstr);
		return FALSE;
	}
