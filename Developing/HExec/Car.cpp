...
extern "C"
{
	__declspec(dllexport) void GetCalls(UCHAR carIndex,CallList *mCallList);
...
// delay before start to run in order to avoid conditional stop
double CCar::GetConditionalDelay()
{
	int inx=curInx;
	int nextInx=stop[inx].next;
	int virtualTarget;
	int tsaDelay;
	int orgLead,tgtLead,orgTrail,tgtTrail;
...
void CCar::SendTarget()
{
	...
	//add stopType for passenger perception purpose
	cmd.stopType=GetStopType(tmpCurInx);
	...
	if(oper!=CAS && oper!=CAS_DECEL && cmd.target!=actual_pos)
	{
		//this car should move and other car is moving
		if(otherCar->oper==CAS || otherCar->oper==CAS_DECEL)
		{
			//if same direction
		...
		if(GetStopType(nextInx)==STOP_TYPE_CONDITIONAL)
		{
	...
	if((oper==CAS || oper==CRS) && (otherCar->oper==CAS || otherCar->oper==CRS || otherCar->oper==CAS_DECEL) && cmd.target!=target_pos && target_pos!=prevFloor)
	{
		...
		if(thisDir==otherDir)
		{
				...
				(thisDir==DN && this->actual_pos>otherCar->actual_pos && cmd.target<=otherCar->prevFloor && cmd.target<target_pos))
			{
				...
				{
					cmd.target=target_pos;
					cmd.stopType=STOP_TYPE_PARKING;
					cmd.pre_dir=NO_DIR;
					cmd.frontDoorCommand=DOOR_NOT_OPEN;
				}
			}
	...
	if(cmd.stopType==255)
	{
		if(typeInx!=-1)
		{
			cmd.stopType=GetStopType(typeInx);
		}
		else
			cmd.stopType=GetStopType(tmpCurInx);
	}
...
void CCar::CallProcessing()
{
	...
	UCHAR deBoardCnt=RemoveCarCalls(carId,actual_pos);
	if(bDispatcherAttached)
	{
		...
		if(bNeedCancelCarCall)
			GDCancelCarCalls(carId,actual_pos);
		index=AssignedCalls.waitingAtHead[actual_pos];
		if(index>=0)
			GDCancelHallCalls(carId,actual_pos);
	}
...
{
	bool ret=false;
	if(stop[inx].type==STOP_TYPE_DEMAND)
		ret=true;
