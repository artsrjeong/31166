...
void CCar::SendTarget()
{
	...
	if((oper==CAS || oper==CRS) && cmd.target!=target_pos && target_pos!=prevFloor)
	{
...
void CCar::CallProcessing()
{
	...
	while(index>=0 && spareCap>0)
	{
		...
		if(AssignedCalls.calls[index].origin==actual_pos && dir==pre_dir)
		{
...
void CCar::Init()
{
	...
	motion.SetMotionParam(pBuilding->nLandings,carInfo.maxJerk,carInfo.maxAcceleration,carInfo.maxVelocity,carInfo.tStartDelay,carInfo.tStopDelay);
