			...
			if ( committed[s].floor == origin) {
				...
				else 
				{
					...
					if (committed[s2].empty)							// if it is an empty move to the next stop, consider as no direction
						dir2 = NO_DIR;
					if ( (dir == dir2) || (dir2 == NO_DIR))  // merge the call if the same directionor no direction
					{
						if (  committed.check_and_merge(assigned_calls, i, capacity))
						{
							committed.UpdateOnBoard();						// Update On-board info	
							i = assigned_calls.RemoveFromList(prev_i);      // remove the call from the list	
							flag = true;
	...
	while (s2 != -1)
	{
		...
		while ( i != -1)
		{
			...
			if (call_dir == dir)
			{
				...
				if ( (UP == call_dir) && (f1 <= origin) && (f2 >= origin)&& committed.check_and_merge(assigned_calls, i, capacity))
				{
					
					// assigned_calls.MoveOriginForward(i, origin);	// Move calls with the same origin forward
					i = assigned_calls.RemoveFromList(prev_i);      // remove the call from the list	
					s2 = committed[s1].next;				// update the new s2
					f2 = committed[s2].floor;
					
				}
				else if ( (DN == call_dir) && (f1 >= origin) && (f2 <= origin) && committed.check_and_merge(assigned_calls, i, capacity))
				{
					// assigned_calls.MoveOriginForward(i, origin);	// Move calls with the same origin forward	
					i = assigned_calls.RemoveFromList(prev_i);      // remove the call from the list	
					s2 = committed[s1].next;				// update the new s2
