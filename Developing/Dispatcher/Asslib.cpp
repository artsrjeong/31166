...
/**********************************************************************************************************
;  Name: asslib_CheckEzRunTime - Check if there is Express Zone between pos1 and pos2, return the EzRunTime
;
;  Description:
;
;      input             Value
...
;      pos2      
;***********************************************************************************************************/
BYTE_T asslib_CheckEzRunTime(BYTE_T car, BYTE_T pos1, BYTE_T pos2)
{
  BYTE_T lowPos,highPos;
  BYTE_T runRrt;
   ...
   if ((ocss_info[car].ez_type == EZ_STANDARD) &&
       ((ocss_info[car].ez_bottom_pos > lowPos) &&
           (ocss_info[car].ez_top_pos < highPos)))
       runRrt += ocss_info[car].ez_runtime;

   if ((ocss_info[car].ez_type2 == EZ_STANDARD) &&
       ((ocss_info[car].ez_bottom_pos2 > lowPos) &&
           (ocss_info[car].ez_top_pos2 < highPos)))
       runRrt += ocss_info[car].ez_runtime2;

   if ((ocss_info[car].ez_type3 == EZ_STANDARD) &&
       ((ocss_info[car].ez_bottom_pos3 > lowPos) &&
           (ocss_info[car].ez_top_pos3 < highPos)))
       runRrt += ocss_info[car].ez_runtime3;
...
FLAG_T asslib_ValidExpectCalls(BYTE_T car,BYTE_T dir,BYTE_T pos,BYTE_T *expectCnts)
{
  FLAG_T  validFlag = 0;
  ...
  if (dir == UP_DIR)
    {
      start = carPos;
      finish = pos - 1;
      dest_mask = DBD_SUCC | DBD_EUCC;
    }
  else
    {
      start = pos + 1;
      finish = carPos;
      dest_mask = DBD_SDCC | DBD_EDCC;
    }

  for (flr = start; flr <= finish; flr++)
    {
       if ((asslib_DbdAssignCalls[car][flr][pos] & dest_mask) != 0)  // is car are call assigned  already
         {
            validFlag = TRUE;
            //break;
            //DBD-honma 07Jul2004
            *expectCnts = *expectCnts + asslib_DbdAssignCallDstCnts[car][flr][pos];
         }
    }
...
/*********************************************************************
; Function Name: asslib_assign_local_call
;
; This task is responsible for assigning a local hall call to the specified
; car (Local: only internal to GCB, not sent on ring yet).
;
; Parameters: car - car being assigned
;             callmask - bitmask of call being assigned
;             pos - floor positiong of call being assigned
;********************************************************************/
void asslib_assign_local_call(BYTE_T car, WORD_T callmask, BYTE_T pos)
{
   ocss_info[car].assigned_calls[pos] |= callmask;

   // this is needed so ocss does not remove a hdcp call from its local call buffer
   // by shadowing a std call into assigned calls. ocss must not turn off a hdcp bit
   if ((callmask & E_HC) != 0) {
      ocss_info[car].assigned_calls[pos] |= ((callmask & E_HC) >> 8);
   }
...
//DBD
/*********************************************************************
;  Function Name: asslib_getassignedcar
;
;********************************************************************/

BYTE_T asslib_GetAssignedCar(BYTE_T callPos, BYTE_T callDst, BYTE_T callMask, BYTE_T riser)
{
  BYTE_T  car,assignedCar = 0;

  for (car=CAR1; car<=CAR8; car++)
      {
        if ( ((ocss_info[car].assigned_calls[callPos] & callMask) != 0) &&
